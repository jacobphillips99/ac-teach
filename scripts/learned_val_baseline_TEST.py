...
def setup():
    ...
    sess = tf.Session()
    rl_model = imp.load_source(
        "rl_modea", '/home/jacobphillips/sandbox/ac-teach/models/rl/learned_value.py').Model(sess=sess, trainable=True)
    discrim_model = imp.load_source(
        "discrim_model", "/home/jacobphillips/sandbox/ac-teach/models/supervised/recurrent_frame_discriminator.py").Model(sess=sess, trainable=True)
...
class ActorDataHolder:
    ...
    def append(self, x, y, y_prev, pred):
        self.xs.append(x)
        self.ys.append(y)
    ...
    def package_data(self):
        return np.stack(self.xs), np.array(self.ys), np.array(self.ys_prev), np.array(self.preds)
...
def repeat_rollout(rl_model, env, actions):
    ...
    for action in actions:
        ...
        # env.render()
        rend = env.env.sim.render(
            500, 500, camera_name='external_camera_0')[::1]
...
if __name__ == "__main__":
    ...
    rl_log_dir, rl_save_dir, discrim_log_dir, discrim_save_dir, folder_time = create_save_log_dirs(
        rl_model)
    ...
    for episode_i in range(1000000):
        ...
        rl_data.reset()
        im_data.reset()
        scores, states, actions, rs, value_preds = [], [], [], [], []
        renders = []
        step = 0
        EVAL = False
        ...
        while not done:  # or for i steps in episode?
            if np.random.rand() > const or episode_i % 10 == 0:
                rl_feed_inf = rl_model.create_rl_feed_dict_inference(
                    obs, action_prev)
                action, value_pred = rl_model.inference(rl_feed_inf)
            ...
            # rs.append(reward)
            rl_data.append(obs, action, action_prev, value_pred)

            step += 1

            if done:
                # get evaluation from discrim model
                discrim_feed = discrim_model.create_discrim_feed_inference(
                    states, actions)
                sigmoids, logits = discrim_model.inference(discrim_feed)
                scores.extend([sigmoid[0][0] for sigmoid in sigmoids])
                print("ep " + str(episode_i) + " with return " + str(sum(rs)) + " with " +
                      str(100*float(actor_action_count)/len(rs))[:3] + "% actor agency")

                if sum(rs) > best_so_far:
                    best_so_far = sum(rs)
                    print(
                        "EPISODE {} BEST SO FAR! SAVING CHECKPOINT AND IMAGES".format(episode_i))
                    print("RS WAS BEST ")
                    print(rs)
                    actor_percent = str(
                        100*float(actor_action_count)/len(rs))[:5]
                    print("ACTOR ACTION % IS " + actor_percent)

                    # repeat the last env to get video !!!
                    images = repeat_rollout(rl_model, env, actions)
                    path = "./videos/" + folder_time + "/" + \
                        str(episode_i)+'_actor_' + actor_percent + '%'
                    os.makedirs(path)
                    for steps, image in enumerate(images):
                        save_image(image, path=path +
                                   "/{}.png".format(str(steps)))

                    rl_model.write_checkpoint(
                        rl_save_dir, name="model-{}".format(int(sum(rs))))
                    discrim_model.write_checkpoint(
                        discrim_save_dir, name="model-{}".format(episode_i))
                ...
                if episode_i % 10 == 0:
                    print("\nEVAL EPISODE " + str(episode_i) + "with return " +
                          str(sum(rs)) + "\t with best so far " + str(best_so_far) + "\n")
                if episode_i % 100 == 0 and best_so_far!=sum(rs): 
                    # repeat the last env to get video !!!
                    images = repeat_rollout(rl_model, env, actions)
                    path = "./videos/" + folder_time + "/" + \
                        str(episode_i)+'_actor_' + actor_percent + '%'
                ...
                rl_feed_train = rl_model.create_rl_feed_dict_train(
                    rl_data, scores, keep_prob=1.0)
                train_step, value_error = sess.run([rl_model.train_step, rl_model.value_error], rl_feed_train)
                ...
                expert_obs, expert_actions = run_SLOW_expert_rollout(
                    teacher, env, GOAL, render=render)
                im_data.extend(expert_obs, expert_actions, [None for i in range(len(expert_actions))])
                discrim_report_rl, discrim_report_il = discrim_model.run_discrim_training(
                    rl_data, im_data)
                rl_correct, il_correct = discrim_model.analyze_discrim_report(
                    discrim_report_rl, discrim_report_il)

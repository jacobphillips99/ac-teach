...
def run_interpolated_expert_rollout(env, goal, num_steps=100, log_path=".", render=False):
    ...
    print("Returned expert rollout with {} steps.".format(len(interp_expert_obs)))
...
def interpolate_expert(expert_actions, expert_obs):
    ...
    num_samples = np.random.randint(75, 100)
    ...
    for episode_i in range(1000000):
        ...
        while not done: #or for i steps in episode?
            ...
            if done:
                ...
                #anneal rewards and scores
                scores_constant = 1
                reward_constant = 1/1000.
                annealed = reward_constant*np.array(rs) + scores_constant*np.array(scores)

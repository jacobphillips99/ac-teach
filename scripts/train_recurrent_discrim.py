...
def setup():
    ...
    env = gym.make("OneGoalPickPlaceSparseEnv-v0")
    sess = tf.Session()
...
def run_expert_rollout(env, goal, num_steps=100, log_path=".", render=False):
    ...
    return obs, expert_actions, ret

def run_SLOW_expert_rollout(env, goal, num_steps=100, log_path=".", render=False):
    env.reset()
    ob = env.env.reset(repeat=False)
    teacher = OptimalPickPlaceAgent(goal=goal)
    done = False
...
if __name__ == "__main__":
    ...
    obs, acts, ret = run_expert_rollout(env, GOAL)
    print("expert rollout" + str(ret) + "  return in " + str(len(obs)) + " steps\n")
    obs, acts, ret = run_SLOW_expert_rollout(env, GOAL)
    print("slow rollout: " + str(ret) + " return in " + str(len(obs)) + " steps")
    import pdb; pdb.set_trace() 
    ...
    for episode_i in range(1000000):
        ...
        while not done: #or for i steps in episode?
            ...
            if done:
                ...
                expert_obs, expert_actions = run_SLOW_expert_rollout(env, GOAL, render=render)

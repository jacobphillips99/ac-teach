...
class Model(AbstractModel):
    ...
    def __init__(self,
        ...
        if trainable:
            ...
            # self.angle_regularizer = 1 * tf.abs(self.joints_sample)

            self.loss = 1*tf.reduce_mean(self.product) #+ 0*tf.reduce_mean(self.angle_regularizer)

            optimizer = tf.train.AdamOptimizer(1e-3) #1e-4              #CONFIG.learning_rate)
    ...
    def inference(self, feed):
        joints_sample = self.sess.run(self.joints_sample, feed_dict=feed)
        return joints_sample[0][0]


    def train_iter(self, feed):
        (_, joints, joints_sample, mus, sigmas, loss, product, neg_log_prob) = self.sess.run([self.train_step, self.joints_, self.joints_sample, self.mus, self.sigmas, self.loss, self.product, self.neg_log_prob], feed_dict=feed)
        report = {'joints':joints, 'joints_sample':joints_sample, 'mus':mus, 'sigmas':sigmas, 'loss':loss, 'product':product, 'neg_log_prob': neg_log_prob}
        return report

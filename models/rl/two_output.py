...
class Model(AbstractModel):
    ...
    def __init__(self,
        ...
        self.tf_xs = tf.placeholder(tf.float32, shape=INPUT_OBS_SHAPE, name='obs')
        self.tf_ys = tf.placeholder(tf.float32, shape=[None, 4], name='control')
        self.tf_rs = tf.placeholder(tf.float32, shape=[None, 1], name='rewards')

        self.tf_ys_prev = tf.placeholder(tf.float32, shape=[None, 4], name='control_prev')
        self.keep_prob = tf.placeholder(tf.float32)
        self.tf_value_truths = tf.placeholder(tf.float32, shape=[None, 1])
        self.tf_value_preds = tf.placeholder(tf.float32, shape=[None, 1])
        ...
        self.fc1 = tf.layers.dense(self.tf_xs, units=2048, activation=tf.nn.relu)
        self.fc1_drop = tf.nn.dropout(self.fc1, self.keep_prob)

        self.fc2 = tf.layers.dense(self.fc1_drop, units=512, activation=tf.nn.relu)
        self.fc2_drop = tf.nn.dropout(self.fc2, self.keep_prob)

        self.quad_normal_params = tf.layers.dense(self.fc2_drop, units=8, activation=None)
        self.mus, self.log_sigmas = tf.split(self.quad_normal_params, [4, 4], axis=1)
        ...
        self.value_dist = tf.distributions.Normal(loc=self.value_mu, scale=self.value_sigma)
        ...
        if trainable:
            ...
            self.value_error = tf.compat.v1.losses.mean_squared_error(self.tf_value_truths, self.tf_value_preds)
            self.loss = 1*tf.reduce_mean(self.product) + 1.0*self.value_error
    ...
    def init_summaries(self, logdir):
        ...
        tf.summary.scalar("loss", self.loss)
            optimizer = tf.train.AdamOptimizer(1e-4)  # CONFIG.learning_rate)
            self.gradients, variables = zip(*optimizer.compute_gradients(self.loss))
            self.gradients, _ = tf.clip_by_global_norm(self.gradients, 5.0)
    ...
    def train_iter(self, feed):
        (_, joints, joints_sample, mus, sigmas, loss, product, neg_log_prob, value_sample, value_, value_error) = self.sess.run(
            [self.train_step, self.joints_, self.joints_sample, self.mus, self.sigmas, self.loss, self.product, self.neg_log_prob, self.value_sample, self.value_, self.value_error], feed_dict=feed)
        report = {'joints': joints, 'joints_sample': joints_sample, 'mus': mus, 'sigmas': sigmas, 'loss': loss, 'product': product, 'neg_log_prob': neg_log_prob, 'value_': value_, 'value_sample': value_sample, 'value_error': value_error)}
        return report
    ...
    def create_rl_feed_dict_inference(self, obs, action_prev, keep_prob = 1.0):
        return {
	        ...
	        self.tf_value_truths: discounted_rewards,
	        self.tf_value_preds: rl_data.preds
        }
